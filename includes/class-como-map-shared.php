...
class Como_Map_Shared {
	 ...
	 */
	public function get_maps( $params, $cache = '' ) {
		$return 	= '';
		$cache_name = $this->plugin_name . '_map_posts';
		if ( ! empty( $cache ) ) {
			$cache_name .= '_' . $cache;
		}
		$return = wp_cache_get( $cache_name, $this->plugin_name . '_map_posts' );
		if ( false === $return ) {
			...
			if ( is_wp_error( $query ) ) {
				...
				wp_cache_set( $cache_name, $query->posts, $this->plugin_name . '_map_posts', 5 * MINUTE_IN_SECONDS );
				$return = $query->posts;
			}
		}
		return $return;
	...
	private function set_args( $params ) {
		...
		if ( ! empty( $params['location'] ) ) {
			$args['meta_query'][]['key'] 		= 'map-location';
