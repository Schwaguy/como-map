 ...
 *
 * @package    Como map
 * @subpackage Como map/includes
 */
class Como_Map_Sanitize {
	/**
	...
	public function set_type( $type ) {
		$check = '';
		if ( empty( $type ) ) {
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'como-map' ) );
		}
		if ( is_wp_error( $check ) ) {
			wp_die( $check->get_error_message(), __( 'Forgot data type', 'como-map' ) );
		}
		$this->type = $type;
	} // set_type()
} // class